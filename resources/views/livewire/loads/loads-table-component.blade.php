  ...
  <div class="lg:col-span-3 p-3">
    <div class="bg-white shadow overflow-hidden sm:rounded-md">
      <ul role="list" class="divide-y divide-gray-200">
        ...
        <li>
          <a href="{{ route('loads.show', $load) }}" class="block hover:bg-gray-50">
              ...
              <div class="min-w-0 flex-1 flex items-center">
                ...
                <div class="min-w-0 flex-1 px-4 md:gap-4">
                  <div class="flex justify-between">
                    <p class="text-sm font-medium text-gray-700 truncate">{{ $load->code }}</p>
                    <p class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-green-100 text-green-800"> {{$load->state}}</p>
                  </div>
                  <div class="flex justify-between">
                    <div>
                      <div class="flex">
                        <div class="mr-2">
                          <div class="mt-1 flex text-sm text-gray-700">
                            ...
                            <div class=" text-sm">
                              <label for="candidates" class="font-medium text-gray-700">Pickup Start</label>
                              <p class="font-medium text-gray-700">{{ $load->pickup_start->format('F d, Y H:m')}}</p>
                            </div>
                          </div>
                        </div>
                        <div>
                          <div class="mt-1 flex text-sm text-gray-700">
                            ...
                            <div class=" text-sm">
                              ...
                              <p class="font-medium text-gray-700">{{ $load->delivery_end->format('F d, Y H:m')}}</p>
                            </div>
                          </div>
                        </div>
                      </div>
                    ...
                    <div class="md:block">
                      <div>
                        <p class="mt-2 text-sm text-gray-700 text-right">
                          Expired Date:
                          <time datetime="2020-01-07">{{ $load->expiry->format('F d, Y H:m')}}</time>
                        </p>
                        <div class="flex">
