<div>
    <div class="relative flex bg-white p-2 space-x-1">
        ...
        <div class="w-64">
            ...
            <x-form.input type="date" name="pickup" wire:model.defer="dateStartString"></x-form.input>
        </div>
        <div class="w-64">
            <label for="first-name" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                Delivery End
            </label>
            <x-form.input type="date" name="delivery" wire:model.defer="dateEndString"></x-form.input>
        </div>
        <div class="w-64">
            <label for="first-name" class="block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                Load type
            </label>
            <x-form.select :options="['all' => 'All types'] + \App\Enums\LoadTypeEnum::labels()" wire:model.defer="loadType"></x-form.select>
        </div>
        <div class="w-40">
            <label for="first-name" class="invisible block text-sm font-medium text-gray-700 sm:mt-px sm:pt-2">
                Search
            </label>
            <x-form.button class="w-full" buttonType="primary" wire:click="searchLoad">Search</x-form.button>
        </div>
    </div>
                ...
                <div class="grid grid-cols-1 gap-y-10 auto-rows-min md:grid-cols-3 md:gap-x-6">
                    <!-- <fieldset>
                        ...
                        <div class="pt-6 space-y-6 sm:pt-4 sm:space-y-4">
                            @foreach(\App\Enums\LoadTypeEnum::labels() as $key => $label)
                            <div class="flex items-center text-base sm:text-sm">
                                <x-form.checkbox name="loadType[]" wire:model="loadType.{{ $key }}" :value="$label">
                                    {{ $label }}
                                </x-form.checkbox>
                            </div>
                            @endforeach
                    ...
                    <fieldset>
                        ...
                        <div class="pt-6 space-y-6 sm:pt-4 sm:space-y-4">
                            @foreach($regions as $key => $region)
                                <div class="flex items-center text-base sm:text-sm">
                                    <x-form.checkbox labelId="pickup-{{ $key }}" name="pickupRegion[]" value="{{ $region->province }}" wire:model.defer="pickupRegion">
                                        {{ $region->province }}
                                    </x-form.checkbox>
                                </div>
                            @endforeach
                    ...
                    <fieldset>
                        ...
                        <div class="pt-6 space-y-6 sm:pt-4 sm:space-y-4">
                            @foreach($regions as $key => $region)
                                <div class="flex items-center text-base sm:text-sm">
                                    <x-form.checkbox labelId="delivery-{{ $key }}" name="deliveryRegion[]" value="{{ $region->province }}" wire:model.defer="deliveryRegion">
                                        {{ $region->province }}
                                    </x-form.checkbox>
                                </div>
                            @endforeach
                    ...
                    <fieldset>
                        ...
                        <div class="pt-6 space-y-6 sm:pt-4 sm:space-y-4">
                            @foreach($statusData as $key => $value)
                                <div class="flex items-center text-base sm:text-sm">
                                    <input  wire:model.defer="statuses.{{ $key }}"  id="statuses-{{ $key }}" name="statuses[]" value="{{ $key }}" type="checkbox" class="flex-shrink-0 h-4 w-4 border-gray-300 rounded text-indigo-600 focus:ring-indigo-500">
                                    <label for="statuses-{{ $key }}" class="ml-3 min-w-0 flex-1 text-gray-600">
                                        {{ $value }}
                                    </label>
                                </div>
                            @endforeach
