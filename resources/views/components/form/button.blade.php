@props([ 'buttonType' => [], 'wireTarget' => ''])

@php

    $buttonClass = match ($buttonType) {
        ...
        'danger' => "inline-flex justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-red-600 hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
    };
@endphp

<button {{ $attributes->merge(['class' => $buttonClass]) }}>
    <svg wire:loading wire:target="{{ $wireTarget ?? '' }}" class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
